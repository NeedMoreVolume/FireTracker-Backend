...
func NewCreateFire(body *CreateRequestBody) *fire.Fire {
	...
	if body.Logs != nil {
		...
		for i, val := range body.Logs {
			v.Logs[i] = unmarshalLogRequestBodyToFireLog(val)
		}
	}
	if body.Weathers != nil {
		v.Weathers = make([]*fire.Weather, len(body.Weathers))
		for i, val := range body.Weathers {
			v.Weathers[i] = unmarshalWeatherRequestBodyToFireWeather(val)
		}
	}
...
func ValidateCreateRequestBody(body *CreateRequestBody) (err error) {
	...
	for _, e := range body.Logs {
		if e != nil {
			if err2 := ValidateLogRequestBody(e); err2 != nil {
	...
	for _, e := range body.Weathers {
		if e != nil {
			if err2 := ValidateWeatherRequestBody(e); err2 != nil {
				err = goa.MergeErrors(err, err2)
			}
		}
	}
