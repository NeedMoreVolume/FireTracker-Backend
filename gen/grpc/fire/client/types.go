...
func NewCreateRequest(payload *fire.Fire) *firepb.CreateRequest {
	...
	if payload.Logs != nil {
		message.Logs = make([]*firepb.Log, len(payload.Logs))
		for i, val := range payload.Logs {
	...
	if payload.Weathers != nil {
		message.Weathers = make([]*firepb.Weather, len(payload.Weathers))
		for i, val := range payload.Weathers {
...
func NewListResult(message *firepb.ListResponse) *fire.FireList {
	...
	if message.Fires != nil {
		...
		for i, val := range message.Fires {
			...
			if val.Logs != nil {
				result.Fires[i].Logs = make([]*fire.Log, len(val.Logs))
				for j, val := range val.Logs {
					result.Fires[i].Logs[j] = &fire.Log{}
					if val.Id != 0 {
						idptr := int(val.Id)
						result.Fires[i].Logs[j].ID = &idptr
					}
					if val.CreatedAt != "" {
						result.Fires[i].Logs[j].CreatedAt = &val.CreatedAt
					}
					if val.UpdatedAt != "" {
						result.Fires[i].Logs[j].UpdatedAt = &val.UpdatedAt
					}
					if val.Name != "" {
						result.Fires[i].Logs[j].Name = &val.Name
					}
					if val.Size != "" {
						result.Fires[i].Logs[j].Size = &val.Size
					}
					if val.FireId != 0 {
						fireIDptr := int(val.FireId)
						result.Fires[i].Logs[j].FireID = &fireIDptr
					}
					if val.AddedAt != "" {
						result.Fires[i].Logs[j].AddedAt = &val.AddedAt
					}
					if val.Weather != nil {
						result.Fires[i].Logs[j].Weather = protobufFirepbWeatherToFireWeather(val.Weather)
					}
				}
			}
			if val.Weathers != nil {
				result.Fires[i].Weathers = make([]*fire.Weather, len(val.Weathers))
				for j, val := range val.Weathers {
					result.Fires[i].Weathers[j] = &fire.Weather{}
					if val.Id != 0 {
						idptr := int(val.Id)
						result.Fires[i].Weathers[j].ID = &idptr
					}
					if val.CreatedAt != "" {
						result.Fires[i].Weathers[j].CreatedAt = &val.CreatedAt
					}
					if val.FireId != 0 {
						fireIDptr := int(val.FireId)
						result.Fires[i].Weathers[j].FireID = &fireIDptr
					}
					if val.LogId != 0 {
						logIDptr := int(val.LogId)
						result.Fires[i].Weathers[j].LogID = &logIDptr
					}
					if val.Humidity != 0 {
						result.Fires[i].Weathers[j].Humidity = &val.Humidity
					}
					if val.WeatherType != "" {
						result.Fires[i].Weathers[j].WeatherType = &val.WeatherType
					}
					if val.WeatherTime != "" {
						result.Fires[i].Weathers[j].WeatherTime = &val.WeatherTime
					}
					if val.Temperature != nil {
						result.Fires[i].Weathers[j].Temperature = protobufFirepbTemperatureToFireTemperature(val.Temperature)
					}
					if val.DewPoint != nil {
						result.Fires[i].Weathers[j].DewPoint = protobufFirepbTemperatureToFireTemperature(val.DewPoint)
					}
					if val.Wind != nil {
						result.Fires[i].Weathers[j].Wind = protobufFirepbWindToFireWind(val.Wind)
					}
				}
			}
		}
